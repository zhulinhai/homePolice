...
-( void )readerView:( ZBarReaderView *)readerView didReadSymbols:( ZBarSymbolSet *)symbols fromImage:( UIImage *)image
{
    ...
    } else if( (symbolStr.length > 0 && symbolStr.length < 20) || symbolStr.length == 5) {
        if (![FieldValidate validatePhone:symbolStr]) {
            dnsName = symbolStr;
        }
    } else if (symbolStr.length == 20) {
        uid = symbolStr;
    }
    
    if (dnsName != nil && uid != nil) {
        AddDeviceViewController *addController = self.delegate;
        addController.userTextField.text = dnsName;
        addController.nameTextField.text = @"摄像机";
        NSMutableDictionary *dict = [NSMutableDictionary dictionary];
        [dict setValue:dnsName forKey:PARSER_DDNS_USER];
        [dict setValue:uid forKey:PARSER_DEVICE_UID];
        addController.deviceInfo = dict;
        [self back:nil];
    }
}
