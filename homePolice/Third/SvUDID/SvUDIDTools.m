...
static const char kKeyChainUDIDAccessGroup[] = "UL8J9TGJ7L.*";
...
{
    NSMutableDictionary *dictToDelete = [[[NSMutableDictionary alloc] init] autorelease];
...
+ (BOOL)updateUDIDInKeyChain:(NSString*)newUDID
{
    ...
    if (queryResult) {
        ...
        NSInteger length = 0;
        if (udidStr != nil) {
            length = strlen(udidStr);
        }
        NSData *keyChainItemValue = [NSData dataWithBytes:udidStr length:length];
